...
const getProducts = async (req, res, next) => {
  try {
    ...
    // If search includes category names, also search by category
    if (req.query.search) {
      const searchTerm = req.query.search;
      ...
      // Filter out products where category didn't match
      ...
      // Re-sort and limit combined results
...
const createProduct = async (req, res, next) => {
  try {
    ...
    // Images are now Cloudinary URLs, no need to modify them
    // Just ensure images is an array
...
const createProductAdmin = async (req, res, next) => {
  try {
    ...
    // Remove empty SKU to avoid duplicate key errors
    if (productData.sku === '') {
      delete productData.sku;
    }
    
    // Images are already full URLs from upload endpoint
